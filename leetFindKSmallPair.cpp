...
//Time Complexity: O(k * log(min(n, m, k))), where n is the size of num1, and m is the size of num2.
...
//Solution 1:
// This can be done using Max Heap and iterate through out the loop. It would take O((m*n)logk) time and O(k) space.
// Need to work on it. 
...
public:
    vector<pair<int, int>> kSmallestPairs(vector<int>& nums1, vector<int>& nums2, int k) {
        ...
        while (!q.empty() && pairs.size() < k) {
            ...
            push(i, j + 1);             //Then here it will keep on increasing indices in vector2.   
            if (j == 0) {               //Why is it calling next value in first vector here.
